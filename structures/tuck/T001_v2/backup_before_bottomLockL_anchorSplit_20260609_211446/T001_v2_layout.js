...
function T001_v2_buildBleedPath(spec, grid) {
  ...
  function bottomLockABleed() {
    ...
    const oldDiagJoin = pt(xP2 + S(26.484), yLRFlat - S(8.672));
    const redDiagTop = pt(xP2 + S(30), yBB + S(18.5));
    const redDiagBot = pt(xP2 + S(28.087), yBB + S(24.602));
    const diagDx = redDiagBot.x - redDiagTop.x;
    const diagDy = redDiagBot.y - redDiagTop.y;
    const diagLen = Math.sqrt(diagDx * diagDx + diagDy * diagDy) || 1;
    const targetDiagJoin = pt(redDiagTop.x - (diagDy / diagLen) * b, redDiagTop.y + (diagDx / diagLen) * b);
    const diagBlend = Math.min(1, Math.max(0, (D - 57) / 143));
    const diagJoin = pt(
      oldDiagJoin.x + (targetDiagJoin.x - oldDiagJoin.x) * diagBlend,
      oldDiagJoin.y + (targetDiagJoin.y - oldDiagJoin.y) * diagBlend
    );
