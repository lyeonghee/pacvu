...
function T001_v2_renderSVG(cfg, appState) {
  ...
  <defs>
    ...
    <style>
      ...
      .glue-area { fill:#8c8c8c; opacity:0.8; stroke:none; }
...
function T001_v2_buildGlueFillLayer(spec, grid) {
  ...
  const yTop = grid.yBodyTop + glueSlope;
  const yBottom = grid.yBodyBottom - glueSlope;
  const d = [
    `M ${N(xL)},${N(yTop)}`,
    `L ${N(xR)},${N(grid.yBodyTop)}`,
    `L ${N(xR)},${N(grid.yBodyBottom)}`,
    `L ${N(xL)},${N(yBottom)}`,
    'Z'
  ].join(' ');
